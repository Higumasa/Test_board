    ...
    <?php
	  try {
         ...
         if (isset($_POST["id"]) && !isset($_POST["title"]) && !isset($_POST["contents"])) {
           ...
           $sql='select title,contents from posts where id = ?';
             //prepareメソッドでSQL文の準備
           $sth = $dbh->prepare($sql);
           ...
           if(!isset($_POST["password"]) || $_POST["password"] != 'pass') {
             ...
             $time2=date("Y m d D H i");

               //実行するSQL文を$sqlに格納
             $sql='update posts set title = ?, contents = ? ,date2 = ? where id = ?';
               //prepareメソッドでSQL文の準備
             $sth = $dbh->prepare($sql);
               //prepareした$sthを実行　SQL文の？部に格納する変数を指定
             $sth->execute(array($_POST["title"],$_POST["contents"],$time2,$_POST["id"]));
